...
if( ! class_exists( 'Relic_Fashion_Store_Repeater_Control' ) ) {
    /**
     * Repeater control
     */
    class Relic_Fashion_Store_Repeater_Control extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'relic-fashion-store' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'relic-fashion-store' ), $this->row_label['value'] );
    		}
    			...
    			foreach ( $this->fields as $field ) {
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_style( 'font-awesome', get_template_directory_uri(). '/assets/library/font-awesome/css/font-awesome.min.css', array(), '4.7' );
    					}
    				}
    			}
    		}
            wp_enqueue_script( 'blossom-feminine-repeater', get_template_directory_uri() . '/themerelic/customizers/custom-controls//repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'blossom-feminine-repeater', get_template_directory_uri() . '/themerelic/customizers/custom-controls//repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'relic-fashion-store' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'relic-fashion-store' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="relic-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'relic-fashion-store' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'relic-fashion-store' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'relic-fashion-store' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'relic-fashion-store' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'relic-fashion-store' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'relic-fashion-store' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'relic-fashion-store' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="relic-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'relic-fashion-store' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'relic-fashion-store' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'relic-fashion-store' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'relic-fashion-store' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'relic-fashion-store' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'relic-fashion-store' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'relic-fashion-store' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
